...
def run_fold(args, fold, txt_name) -> Tuple:
    ...
    model = [None]*idx_process
    file_name = [None]*idx_process
    auc_best = [0.0]*idx_process
    epoch_best = [0]*idx_process
    auc_val = [0.0]*idx_process
    acc_val = [0.0]*idx_process
    auc_test = [0.0]*idx_process
    acc_test = [0.0]*idx_process
    auc_tr = [0.0]*idx_process
    acc_tr = [0.0]*idx_process

    for idx_process in range(args.process_num):
        model[idx_process] = milmodels.__dict__[args.mil_model](args=args, ma_dim_in=2048).cuda()
...
if __name__ == '__main__':
    ...
    ################################### 여길 바꿔 ####################################
    args.train_instance = ['interinstance_cosine']*16
    args.ic_num_head = [5]*16
    args.ic_depth = [2]*16
    args.weight_agree = [1.0]*16
    args.weight_disagree = [1.0]*16
    args.lr = [0.003]*16
    args.lr_center = [0.0001]*16
    args.mil_model = ['Dsmil']*16
    args.gpus = list(range(8))*2
    ##################################################################################

    args.process_num = len(args.gpus)
    txt_name = [0]*args.process_num
    acc_fold_tr = [[]]*args.process_num
    auc_fold_tr = [[]]*args.process_num

    acc_fold_val = [[]]*args.process_num
    auc_fold_val = [[]]*args.process_num

    acc_fold_test = [[]]*args.process_num
    auc_fold_test = [[]]*args.process_num

    for idx_process in range(args.process_num):
    
        txt_name[idx_process] = f'{datetime.today().strftime("%m%d")}_{args.dataset}_{args.mil_model}_scheduler_centroid{args.scheduler_centroid}_train_instance{args.train_instance}' +\
    ...
    for idx_process in range(args.process_num):
        ...
        with open(txt_name[idx_process] + '.txt', 'a' if os.path.isfile(txt_name[idx_process] + '.txt') else 'w') as f:
            f.write(f'===================== LR-mil: {args.lr[idx_process]} || LR-negative center: {args.lr_center[idx_process]} =======================\n')
