...
for layernum_head in 1 2; do
    for lr_downstream in 0.01 0.001 0.0001 0.00001; do
        ...
        # optimizer=adamw
        CUDA_VISIBLE_DEVICES=$gpu nohup python train.py --data-root $data_root --layernum-head $layernum_head --stddev-disagree $stddev_disagree --mil-model $milmodel --dataset $dataset --epochs 200 --workers 4 --aux-loss $aux_loss --weight-cov $weight_cov --lr $lr_downstream --pushtoken o.OsyxHt1pZuwUBoMEFYBuzHFNjV5ekr95 > LR_${lr_downstream}_milmodel_${milmodel}_gpu_${gpu}.txt &
