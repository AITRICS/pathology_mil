...
def generate_patch(path_wsi, pkl_dict, args):
    """
    0) Get mpp_np
    1) Get level_mask, size_patch_level0
    2) Get threshold_cb, threshold_cr using level_mask
    3) Do for loop using size_patch_level0
    ...
    size_patch_level0_x = round(args.config[args.dataset]['size_patch'] * args.config[args.dataset]['mpp_patch'] / mppx)
    size_patch_level0_y = round(args.config[args.dataset]['size_patch'] * args.config[args.dataset]['mpp_patch'] / mppy)
    ...
    list_idx_width = list(range(0, w_level0, size_patch_level0_x))
    list_idx_height = list(range(0, h_level0, size_patch_level0_y))

    if w_level0 % size_patch_level0_x != 0:
        list_idx_width.pop(-1)
        list_idx_width.append(w_level0-size_patch_level0_x)
    if h_level0 % size_patch_level0_y != 0:
        list_idx_height.pop(-1)
        list_idx_height.append(h_level0-size_patch_level0_y)

    threshold_background = size_patch_level0_x * size_patch_level0_y * args.threshold_mask
    kernel = np.ones((32, 32), np.uint8)

    ## 4) Do for loop using size_patch_level_patch
    for i in list_idx_width:
        for j in list_idx_height:
            patch_pl = wsi.read_region(location=(i,j), level=0, size=(size_patch_level0_x, size_patch_level0_y))
...
if __name__ == "__main__":
    ...
    args.config = {'camelyon': {'mpp_mask': 30, 'mpp_patch': 0.5, 'size_patch': 224, 'ext': '.tif', 'path_list_wsi': '/mnt/aitrics_ext/ext01/shared/camelyon/CAMELYON16/images/*', 'root_patch': '/mnt/aitrics_ext/ext01/shared/', 'pkl': 'subjectID_camelyon16'},
                'tcga_lung': {'mpp_mask': 2, 'mpp_patch': 0.5, 'size_patch': 224, 'ext': '.svs', 'path_list_wsi': '/mnt/aitrics_ext/ext01/shared/tcga_lung/*/*', 'root_patch': '/mnt/aitrics_ext/ext01/shared/', 'pkl': 'subjectID_tcgalung'},
                'tcga_stad': {'mpp_mask': 2, 'mpp_patch': 1.0, 'size_patch': 448, 'ext': '.svs', 'path_list_wsi': '/mnt/aitrics_ext/ext01/shared/tcga_stad_wsi/*/*', 'root_patch': '/mnt/aitrics_ext/ext01/shared/', 'pkl': 'subjectID_tcgastadher2'}}
