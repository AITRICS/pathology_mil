...
def main():
    ...
    model.fc = nn.Linear(model.fc.in_features, 2) # 무조건 2class로
    model.cuda()

    if args.weights==0.5:
        criterion = nn.CrossEntropyLoss().cuda() # ce loss 쓰네
    else:
    ...
    for epoch in range(args.nepochs):
        ...
        probs = inference(epoch, train_loader, model) # 패치를 몽땅 인퍼런스
        topk = group_argtopk(np.array(train_dset.slideIDX), probs, args.k) # 모든 슬라이드의 top1 ( 즉 max pooling )
        train_dset.maketraindata(topk) # k=1 이라 사실상 의미 없음
        train_dset.shuffletraindata() # k=1 이라 사실상 의미 없음
        train_dset.setmode(2) # top1 패치에 슬라이드 라벨 부여
