...
class MILModel(nn.Module):
    ...
    def __init__(
        ...
        if mil_mode.lower() not in ["mean", "max", "att", "att_trans", "att_trans_pyramid", "att_trans_contrastive_1"]:
        ...
        if backbone is None:
            ...
            for i, (name, layer) in enumerate(net.named_children()):
                if isinstance(layer, nn.ReLU):
                    setattr(net, name, nn.ReLU(inplace=False))
        ...
        elif isinstance(backbone, str): #볼 필요 x
        ...
        elif isinstance(backbone, nn.Module): #볼 필요 x
        ...
        if backbone is not None and mil_mode not in ["mean", "max", "att", "att_trans", "att_trans_contrastive_1"]:
        ...
        elif self.mil_mode in ["att_trans", "att_trans_contrastive_1"]:
    ...
    def calc_head(self, x: torch.Tensor) -> torch.Tensor:
        ...
        elif self.mil_mode == "att_trans_contrastive_1" and self.transformer is not None:

            x = x.permute(1, 0, 2) #permute 후 (n,b,512)
            score = self.myfc(x) # score (n,b,1)
            x = self.transformer(x)
            x = x.permute(1, 0, 2) # (b,n,512)

            a = self.attention(x) # att shape (1,n,1)
            a = torch.softmax(a, dim=1) # att shape (1,n,1)
            x = torch.sum(x * a, dim=1) # x*a shape (1,n,512) , sum 후 (1,512)
            # myfc = nn.Linear(512, num_classes)
            x = self.myfc(x)
        ...
        if self.mil_mode =="att_trans_contrastive_1":
            return x, score
    ...
    def forward(self, x: torch.Tensor, no_head: bool = False) -> torch.Tensor:
        ...
        elif self.mil_mode == "att_trans_contrastive_1":
            if not no_head:
                x, score = self.calc_head(x)
                return x, score
            else :
                return x
