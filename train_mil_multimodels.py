...
def run_fold(args, fold) -> Tuple:
    ...
    if args.agg =='max':
        model = milmodels.__dict__[args.mil_model](dim_out=args.num_classes, pool=nn.AdaptiveMaxPool1d((1))).cuda() 
    elif args.agg == 'mean':
        model = milmodels.__dict__[args.mil_model](dim_out=args.num_classes, pool=nn.AdaptiveAvgPool1d((1))).cuda() 
    if args.agg == 'rnn':
        pass    
